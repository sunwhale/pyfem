...
class Diffusion(BaseElement):
    ...
    def update_element_material_stiffness_fint(self,
        ...
        for i in range(qp_number):
            ...
            if is_update_stiffness:
                self.element_stiffness += 1.0 / dtime * dot(qp_shape_value.transpose(), qp_shape_value) * qp_weight_times_jacobi_det + \
                                          dot(qp_shape_gradient.transpose(), dot(qp_ddsddc, qp_shape_gradient)) * qp_weight_times_jacobi_det
                # self.element_stiffness += dot(qp_shape_gradient.transpose(), dot(qp_ddsddc, qp_shape_gradient)) * \
                #                           qp_weight_times_jacobi_det

            if is_update_fint:
                self.element_fint += 1.0 / dtime * dot(qp_shape_value.transpose(), qp_concentration) * qp_weight_times_jacobi_det + \
                                     dot(qp_shape_gradient.transpose(), qp_concentration_flux) * qp_weight_times_jacobi_det
                # self.element_fint += dot(qp_shape_gradient.transpose(), qp_concentration_flux) * qp_weight_times_jacobi_det
