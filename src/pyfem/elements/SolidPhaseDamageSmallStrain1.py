...
class SolidPhaseDamageSmallStrain(BaseElement):
    ...
    def update_element_material_stiffness_fint(self,
        ...
        for i in range(qp_number):
            ...
            # if dimension == 2:
            #     gi = np.array([1.0 * gc, 1.0 * gc])
            #     qp_stress_tensor = np.array([[qp_stress[0], qp_stress[2]], [qp_stress[2], qp_stress[1]]])
            #
            # stress_on_crack = abs(np.dot(qp_stress_tensor, qp_phase_gradient))
            # if norm(stress_on_crack) < 1e-16:
            #     gn = np.array([0.5, 0.5])
            # else:
            #     gn = stress_on_crack / sum(stress_on_crack)
            # gc_eff = float(np.dot(gn, gi))

            # a = np.dot(qp_stress_tensor, qp_phase_gradient)
            # b = norm(a)
            # if b < 1e-16:
            #     gn = np.array([1, 1]) / sum(np.array([1, 1]))
            # else:
            #     gn = abs(a) / b
            # gc_eff = float(np.dot(gn, gi))
            ...
            if is_update_stiffness:
                ...
                self.element_stiffness[np.ix_(self.dof_p, self.dof_p)] += qp_weight_times_jacobi_det * \
                                                                          ((gc_eff / lc + 2.0 * energy_positive) * np.outer(qp_shape_value, qp_shape_value) +
                                                                           gc_eff * lc * np.dot(qp_dhdx.transpose(), qp_dhdx))
            ...
            if is_update_fint:
                ...
                self.element_fint[self.dof_p] += qp_weight_times_jacobi_det * \
                                                 (gc_eff * lc * np.dot(qp_dhdx.transpose(), (qp_phase_gradient + qp_dphase_gradient)) +
