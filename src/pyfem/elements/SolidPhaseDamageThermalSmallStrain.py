...
class SolidPhaseDamageThermalSmallStrain(BaseElement):
    r"""
    固体相场断裂单元。
    ...
    __slots_dict__: dict = {
        ...
        'qp_energies': ('list[np.ndarray]', '积分点处的相场驱动能量矩阵列表'),
        ...
        'qp_temperatures': ('np.ndarray', '积分点处的温度列表'),
        'qp_dtemperatures': ('np.ndarray', '积分点处的温度增量列表'),
        'qp_heat_fluxes': ('np.ndarray', '积分点处的热流密度列表'),
        'qp_ddsddts': ('list[np.ndarray]', '积分点处的材料热传导系数矩阵列表'),
    ...
    def __init__(self, element_id: int,
        ...
        if self.dimension == 2:
            self.dof_names = ['u1', 'u2', 'phi', 'T']
            self.ntens = 4
            self.ndi = 3
            self.nshr = 1
        elif self.dimension == 3:
            self.dof_names = ['u1', 'u2', 'u3', 'phi', 'T']
            self.ntens = 6
            self.ndi = 3
            self.nshr = 3
        else:
        ...
        self.qp_temperatures: list[np.ndarray] = None  # type: ignore
        self.qp_dtemperatures: list[np.ndarray] = None  # type: ignore
        self.qp_heat_fluxes: list[np.ndarray] = None  # type: ignore
        self.qp_ddsddts: list[np.ndarray] = None  # type: ignore
    ...
    def update_element_material_stiffness_fint(self,
        ...
        thermal_material_data = self.material_data_list[2]
        solid_thermal_material_data = self.material_data_list[3]
        ...
        for i in range(qp_number):
            if is_update_material:
                ...
                qp_temperature = np.dot(qp_shape_value, T)
                qp_dtemperature = np.dot(qp_shape_value, dT)
                qp_temperature_gradient = np.dot(qp_dhdx, T)
                qp_dtemperature_gradient = np.dot(qp_dhdx, dT)
                ...
                # self.qp_temperatures.append(qp_temperature)
                # self.qp_dtemperatures.append(qp_dtemperature)
                # self.qp_heat_fluxes.append(qp_temperature_gradient + qp_dtemperature_gradient)
            ...
            if is_update_stiffness:
                self.element_stiffness[np.ix_(self.dof_u, self.dof_u)] += qp_weight_times_jacobi_det * \
                                                                          np.dot(qp_b_matrix_transpose, np.dot(qp_ddsdde * qp_degradation, qp_b_matrix))

                self.element_stiffness[np.ix_(self.dof_p, self.dof_p)] += qp_weight_times_jacobi_det * \
                                                                          ((gc / lc + 2.0 * energy_positive) * np.outer(qp_shape_value, qp_shape_value) +
                ...
                self.element_stiffness[np.ix_(self.dof_T, self.dof_T)] += np.dot(qp_dhdx.transpose(), np.dot(qp_ddsddt, qp_dhdx)) * qp_weight_times_jacobi_det

            if is_update_fint:
                self.element_fint[self.dof_u] += np.dot(qp_b_matrix_transpose, qp_stress * qp_degradation) * qp_weight_times_jacobi_det
                ...
                self.element_fint[self.dof_T] += np.dot(qp_dhdx.transpose(), qp_heat_flux) * qp_weight_times_jacobi_det

                dsdt = -1.0 * np.dot(qp_ddsdde, alpha)
                self.element_stiffness[np.ix_(self.dof_u, self.dof_T)] += \
                    np.dot(qp_b_matrix_transpose, np.outer(dsdt, qp_shape_value)) * qp_weight_times_jacobi_det
    ...
    def update_element_field_variables(self) -> None:
        ...
        self.qp_field_variables['heat_flux'] = np.array(self.qp_heat_fluxes, dtype=DTYPE)
        for key in self.qp_state_variables_new[0].keys():
            if key not in ['strain', 'stress', 'energy', 'heat_flux']:
                variable = []
...
if __name__ == "__main__":
    ...
    print_slots_dict(SolidPhaseDamageThermalSmallStrain.__slots_dict__)
