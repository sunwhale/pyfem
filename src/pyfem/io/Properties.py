...
class Properties:
    ...
    def verify(self) -> None:
        ...
        is_error = False
        error_msg = '\nInput error:\n'
        for key in self.__slots__[2:]:  # 忽略这2个关键字：'work_path', 'input_file'，它们不是在.toml中定义的
    ...
    def set_mesh(self, mesh_dict: Dict) -> None:
        self.mesh = Mesh()
        allowed_keys = self.mesh.__slots__
    ...
    def set_dofs(self, dofs_dict: Dict) -> None:
        self.dof = Dof()
        allowed_keys = self.dof.__slots__
    ...
    def set_solver(self, solver_dict: Dict) -> None:
        self.solver = Solver()
        allowed_keys = self.solver.__slots__
    ...
    def set_materials(self, materials_list: List) -> None:
        ...
        for material_dict in materials_list:
            material = Material()
            allowed_keys = material.__slots__
    ...
    def set_sections(self, sections_list: List) -> None:
        ...
        for section_dict in sections_list:
            section = Section()
            allowed_keys = section.__slots__
    ...
    def set_amplitudes(self, amplitudes_list: List) -> None:
        ...
        for amplitude_dict in amplitudes_list:
            amplitude = Amplitude()
            allowed_keys = amplitude.__slots__
