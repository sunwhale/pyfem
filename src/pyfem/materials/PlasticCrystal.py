...
class PlasticCrystal(BaseMaterial):
    ...
    :ivar c_1: 随动强化参数
    :vartype c_1: np.ndarray

    :ivar c_2: 随动强化参数
    :vartype c_2: np.ndarray

    :ivar r_0: 随动强化参数
    :vartype r_0: np.ndarray
    ...
    __slots_dict__: dict = {
        ...
        'theta': ('float', '切线系数法参数'),
        'K': ('np.ndarray', '参考屈服强度'),
        'dot_gamma_0': ('np.ndarray', '参考剪切应变率'),
        'p_s': ('np.ndarray', '强化指数'),
        'c_1': ('np.ndarray', '随动强化参数'),
        'c_2': ('np.ndarray', '随动强化参数'),
        'r_0': ('np.ndarray', '初始阻应力'),
        'b_s': ('np.ndarray', '各项同性强化参数'),
        'Q_s': ('np.ndarray', '各项同性强化参数'),
        'H': ('np.ndarray', '硬化系数矩阵'),
    ...
    def __init__(self, material: Material, dimension: int, section: Section) -> None:
        ...
        for i, (name, ca) in enumerate(zip(self.slip_system_name, self.c_over_a)):
            ...
            K = np.ones((slip_system_number,), dtype=DTYPE) * material.data_dict['K'][i]
            dot_gamma_0 = np.ones((slip_system_number,), dtype=DTYPE) * material.data_dict['dot_gamma_0'][i]
            p_s = np.ones((slip_system_number,), dtype=DTYPE) * material.data_dict['p_s'][i]
            c_1 = np.ones((slip_system_number,), dtype=DTYPE) * material.data_dict['c_1'][i]
            c_2 = np.ones((slip_system_number,), dtype=DTYPE) * material.data_dict['c_2'][i]
            r_0 = np.ones((slip_system_number,), dtype=DTYPE) * material.data_dict['r_0'][i]
            ...
            if i == 0:
                ...
                self.K: np.ndarray = K
                self.dot_gamma_0: np.ndarray = dot_gamma_0
                self.p_s: np.ndarray = p_s
                self.c_1: np.ndarray = c_1
                self.c_2: np.ndarray = c_2
                self.r_0: np.ndarray = r_0
                ...
                self.dot_gamma_0 = np.concatenate((self.dot_gamma_0, dot_gamma_0))
                self.p_s = np.concatenate((self.p_s, p_s))
                self.c_1 = np.concatenate((self.c_1, c_1))
                self.c_2 = np.concatenate((self.c_2, c_2))
                self.r_0 = np.concatenate((self.r_0, r_0))
                self.b_s = np.concatenate((self.b_s, b_s))
                self.Q_s = np.concatenate((self.Q_s, Q_s))

        self.H = np.ones(shape=(self.total_number_of_slips, self.total_number_of_slips), dtype=DTYPE)
    ...
    def get_tangent(self, variable: dict[str, np.ndarray],
        ...
        **获得幂指数形式的晶体塑性本构模型**
        ...
        if state_variable == {} or timer.time0 == 0.0:
            ...
            state_variable['gamma'] = np.zeros(shape=self.total_number_of_slips, dtype=DTYPE)
            state_variable['rho'] = np.zeros(shape=self.total_number_of_slips, dtype=DTYPE)
            state_variable['alpha'] = np.zeros(shape=self.total_number_of_slips, dtype=DTYPE)
            state_variable['r'] = np.zeros(shape=self.total_number_of_slips, dtype=DTYPE) + r_0
        ...
        delta_gamma = np.zeros(shape=self.total_number_of_slips, dtype=DTYPE)
        ...
        for niter in range(self.MAX_NITER):
            ...
            X = (abs(tau - alpha) - r) / K
            gamma_dot = dot_gamma_0 * np.maximum(X, 0.0) ** p_s * np.sign(tau - alpha)

            if niter == 0:
                gamma_dot_t = deepcopy(gamma_dot)

            term1 = dt * theta
            term2 = term1 * dot_gamma_0 * p_s * np.maximum(X, 0.0) ** (p_s - 1.0) / K
            term3 = term1 * np.maximum(X, 0) * dot_gamma_0 * p_s * np.maximum(X, 0.0) ** (p_s - 1.0) / K
            term4 = np.einsum('ik, jk->ij', S, P)

            A = np.eye(self.total_number_of_slips, dtype=DTYPE)
            A += term2 * term4
            # A += H * term3 * sign(gamma_dot) * sign(tau - alpha)
            A += term2 * (c_1 - c_2 * alpha * np.sign(gamma_dot)) * np.eye(self.total_number_of_slips, dtype=DTYPE)
            A += term2 * b_s * Q_s * H * (1.0 - b_s * rho) * np.sign(tau - alpha) * np.sign(gamma_dot)

            if niter == 0:
                rhs = dt * gamma_dot + term2 * np.dot(S, dstrain)
            else:
            ...
            delta_r = b_s * Q_s * np.dot(H, delta_rho)
